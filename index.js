const perguntas = [
  ...
  //o Set não repete por exemplo se esta errado, ele guarda a informação //
  const corretas = new Set()
  const totalDePerguntas = perguntas.length // ele soma a partir do 1 //
  const mostrarTotal = document.querySelector('#acertos span')// o # seria o id acertos, e quero procurar o span, por isso dou espaço e coloco o span //
  ...
  for(const item of perguntas) {
    ...
    for(let resposta of item.respostas) {
      ...
      dt.querySelector('input').onchange = (event) => {
        //aqui é para saber se as respostas estão corretas event.target.value seria uma string e item.correta um numero, number//
        const estaCorreta = event.target.value == item.correta

        corretas.delete(item)// se tiver errada ele deletara //
        if(estaCorreta) { //aqui ele ve se acertou //
            corretas.add(item)
        }
